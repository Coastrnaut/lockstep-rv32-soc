...
begin
  ...
  -- 4. UART TX OUTPUT
  -- ========================================================================
  uart_tx_o <= r_tx_shift(0) when r_uart_st /= uart_idle else
               '1';

  -- Busy flag for diagnostics
  uart_busy_o  <= '1' when r_uart_st /= uart_idle else
                  '0';
  uart_error_o <= '1' when (r_tx_valid = '1' and w_parity_ok = '0') else
                  '0';
